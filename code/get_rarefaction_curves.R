...
# inputs : habitat specific non-subsampled OTU table
...
#   and output data/processed/<16S or ITS>/otu_processed/<RE or RH or BS>_rarefaction_curves.txt
...
rarefaction <- rarecurve(otu_input, step = 100)

# Format results
rarefaction_results <- map_dfr(rarefaction, bind_rows) %>% 
  bind_cols(sample_id = rownames(otu_input), .) %>% 
